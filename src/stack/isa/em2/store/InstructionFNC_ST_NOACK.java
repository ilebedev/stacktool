...
public class InstructionFNC_ST_NOACK extends OneImmediateInstruction implements StoreInstruction{
	
	public InstructionFNC_ST_NOACK(int offset, String comment){
		super(offset, comment);
	}
	
	@Override
	public String toString() {
		return get_string(StackOP.FNC_ST_NOACK, getImmediate(), getComment());
	}

	@Override
	public int toBinary() {
		return assemble(StackOP.FNC_ST_NOACK, getImmediate());
	}
	...
	public void execute(Context context, CoreModel core) throws SimulatorException, UnderflowException, OverflowException {
		...
		// Predict, and decide between FNC_ST_EM_NOACK and FNC_ST_RA_NOACK
		if (predictor.isMigration(address)){
			...
			InstructionFNC_ST_EM_NOACK em = new InstructionFNC_ST_EM_NOACK(stack0size, stack1size, getImmediate(), "");
			em.execute(context, core);
		} else {
			InstructionFNC_ST_RA_NOACK ra = new InstructionFNC_ST_RA_NOACK(getImmediate(), "");
